...
with open("../../parameter/RF_merged_ALS_params-500.json", "r") as f:
    rf_params = json.load(f)
with open("../../parameter/lgbm_best_merged_ALS_params-500.json", "r") as f:
    lgbm_params = json.load(f)
with open("../../parameter/xgboost_best_merged_ALS_params-500.json", "r") as f:
...
for name, clf in models.items():
    ...
    # Save figure
    fig_path = os.path.join(output_dir, f"shap_bar_{name.replace(' ', '_')}_merged_ALS.png")
...
for name, shap_values in shap_results.items():
    ...
    # Save figure
    fig_path = os.path.join(output_dir, f"shap_beeswarm_{name.replace(' ', '_')}_merged_ALS.png")
    plt.savefig(fig_path, dpi=300, bbox_inches='tight')
    plt.show()
    print(f"💾 Saved: {fig_path}")
...
for name, shap_values in shap_results.items():
    ...
    csv_path = os.path.join(output_dir, f"top50_features_shap_{name.replace(' ', '_')}_merged_ALS.csv")
...
csv_output_path = os.path.join(output_dir, "common_features_across_models_merged_ALS.csv")
